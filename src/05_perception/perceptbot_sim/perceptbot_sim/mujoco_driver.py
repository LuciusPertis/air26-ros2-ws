"""mujoco_driver — runs the perception rover in MuJoCo and exposes the full sensor set.

This is the project-02 microbot MuJoCo driver (kinematic planar base, 3 rangefinder
ultrasonics, odom/TF/joint_states) PLUS the new front camera: each frame it renders the
MuJoCo "cam" offscreen and publishes it as /camera/image_raw, so camera_processor and
aruco_detector run identically to Webots / the real ESP32-CAM.

  subscribes:  /cmd_vel
  publishes:   /ultrasonic/front|left|right (Range), /odom + TF, /joint_states,
               /camera/image_raw (Image rgb8) + /camera/camera_info

Headless: set MUJOCO_GL=egl (the launch does). On a display, glfw works too.
"""
...
from sensor_msgs.msg import JointState, Range, Image, CameraInfo
...
class MujocoDriver(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('cam_rate', 15.0)

        mjcf = os.path.join(get_package_share_directory('perceptbot_description'),
                            'mjcf', 'perceptbot.xml')
        ...
        cam_period = max(1, int(self.get_parameter('rate').value /
                                self.get_parameter('cam_rate').value))
        self._cam_period = cam_period
        self._tick = 0
        ...
        self.get_logger().info('mujoco_driver up: /cmd_vel -> perceptbot -> /ultrasonic/*, '
                               '/odom, /camera/image_raw.')

    def _build_camera_info(self):
        info = CameraInfo(width=self.cw, height=self.ch)
        fovy = 60.0 * math.pi / 180.0
        fy = self.ch / (2.0 * math.tan(fovy / 2.0))
        fx = fy
        cx, cy = self.cw / 2.0, self.ch / 2.0
    ...
    def step(self):
        ...
        # === CHECKPOINT: camera ===
        self._tick += 1
        if self._tick % self._cam_period == 0:
            self.publish_camera(now)
        # === END CHECKPOINT: camera ===

    def publish_camera(self, stamp):
        self.renderer.update_scene(self.data, camera='cam')
        rgb = self.renderer.render()                 # HxWx3 uint8 RGB
        msg = Image()
