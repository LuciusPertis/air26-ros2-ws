...
import numpy as np
# === AIR26 WORKSHOP PATCH: make Robocasa optional ===
# Robocasa/robosuite are only needed for the kitchen-environment scenes. The
# workshop (architecture / control / Nav2 / MoveIt2) runs on the plain scene
# (use_robocasa:=false), and these packages are heavy + GPU-hungry. We guard the
# imports so this module loads without them; the robocasa-only functions below
# will raise clearly if called without the packages installed.
try:
    import robosuite
