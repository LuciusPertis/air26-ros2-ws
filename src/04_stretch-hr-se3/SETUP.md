...
git clone --depth 1 -b humble https://github.com/hello-robot/stretch_ros2.git
...
### 2c. Known-good pins  ⚠️ ROS 2 Humble requires numpy 1.x
`uv` and `opencv-python` will happily pull **numpy 2**, which breaks Humble's
`rclpy`/`cv_bridge`/`ros2_numpy`. Hold these versions:

```
numpy==1.24.2             # PIN — Humble is built against numpy 1.x
...
ros2-numpy==0.0.5         # from PyPI — there is NO apt pkg ros-humble-ros2-numpy
...
python3 -m pip install \
  "numpy==1.24.2" "opencv-python==4.10.0.84" "mujoco==3.2.6" \
...
| ros-humble-nav2-amcl | 1.1.20 |
| ros-humble-nav2-map-server | 1.1.20 |
| ros-humble-rplidar-ros | 2.1.4 |
| ros-humble-laser-filters | 2.0.9 |
| ros-humble-tf-transformations | 1.1.1 |
| ros-humble-realsense2-camera (+ description) | 4.57.7 |
| ros-humble-diagnostic-updater (+ aggregator) | 4.0.6 |
| ros-humble-joint-state-publisher (+ gui) | 2.4.0 |
| ros-humble-control-msgs, nav2-* deps, xterm, launch-pytest | (deps) |
...
# (ros-humble-moveit-py does NOT exist for Humble — do not include it)
...
apt pulled (versions on this machine): `ros-humble-moveit*` **2.5.9**, plus `ompl 1.5.2`,
`ros-humble-moveit-resources-*`, `warehouse-ros`/`-mongo` deps.
...
| `No module named 'ros2_numpy'` | no apt pkg `ros-humble-ros2-numpy` | `pip install ros2-numpy` (PyPI) |
| `No module named 'pyquaternion'` | not auto-pulled by hello_helpers | `pip install pyquaternion` |
| opencv wants `numpy>=2` / numpy 2 breaks rclpy | opencv 4.13 forces numpy 2 | pin `opencv-python==4.10.0.84` + `numpy==1.24.2` |
